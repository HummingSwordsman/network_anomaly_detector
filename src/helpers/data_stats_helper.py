...
def __explore_data(data_file_path, data_dir, info, output_file_name_prefix, plot_corr=False, plot_cls_dist=False, plot_attr_dist=False):
    ...
    # Data Correlations
    if plot_corr:
        plot_correlations(df, f"Correlations for {data_file_path}")

    # Data distribution per class
    if plot_cls_dist:
        plot_class_values_distribution(data_dir, df, data_cleanup['classification_col'], title='\n' + info + '\n\n' + "Class Frequency", file_name=output_file_name_prefix + "_class_values_distribution.png", export=True)

    # Data distribution per attribute
    if plot_attr_dist:
        plot_attr_values_distribution(data_dir, df, title='\n' + info + '\n\n' + "Value Distribution", file_name=output_file_name_prefix + "_attr_values_distribution.png", export=True)
